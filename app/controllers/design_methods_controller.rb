...
class DesignMethodsController < ApplicationController
  ...
  def index
      @design_methods = DesignMethod.where.not(completion_score: (0..5).to_a).order(completion_score: :desc)
      ...
      if params[:characteristic] != nil
        ...
        @design_methods = DesignMethod.where(id: list_of_ids).where.not(completion_score: (0..5).to_a).order(completion_score: :desc)
      end
